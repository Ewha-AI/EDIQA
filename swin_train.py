...
for epoch in range(start_epoch, epochs):
    ...
    for i, (data, mean) in enumerate(trainloader):
        ...
        # output = distribution_to_score(list(range(1,6)), output)
        ...
        plcc = calculate_plcc(output, mean)
        ...
        trainloader.set_description("Loss %.04f \t\t PLCC %.04f \t\t Step %d/%d" % (loss, plcc, i, len(train_loader)))
    ...
    with torch.no_grad():
        ...
        for data, mean in tqdm(val_loader, desc='validation'):
            ...
            val_plcc = calculate_plcc(val_output, mean)
