...
def partial_rank_loss(d, y, num_images, eps=1e-6, norm_num=True): # prediction, target
    loss = torch.zeros(1).cuda() #, device=device)
    if num_images < 2:
        return loss

    # sort d and y
    # y = [x for _, x in torch.sort(d, dim=0)]
    d, idx = torch.sort(d, dim=0)
    y = [y[x].squeeze(0) for x in idx]

    dp = torch.abs(d)
    # combinations = torch.combinations(torch.arange(num_images), 2)
    combinations = [[i, i+1] for i in range(num_images-1)]
    combinations_count = max(1, len(combinations))
...
# patch
train_data = MayoRandomPatchDataset2(train_list, label_dir, transform='train', norm=False)
train_loader = DataLoader(dataset = train_data, batch_size=batch_size, shuffle=True, num_workers=4)
val_data = MayoRandomPatchDataset2(val_list, label_dir, transform='val', norm=False)
val_loader = DataLoader(dataset = val_data, batch_size=batch_size, shuffle=False, num_workers=4)
test_data = MayoRandomPatchDataset2(test_list, label_dir, transform='val', norm=False)
test_loader = DataLoader(dataset = test_data, batch_size=batch_size, shuffle=False, num_workers=4)
...
if args.scheduler == 'step':
    ...
    scheduler = ReduceLROnPlateau(optimizer, 'min', factor=0.9, patience=3)
...
if args.transfer == 'swin_conv_detection':
    ...
    for key in list(swin_checkpoint['model'].keys()):
        swin_checkpoint['model']['module.0.swin_transformer.'+key] = swin_checkpoint['model'].pop(key)
    for key in list(conv_checkpoint['model'].keys()):
        conv_checkpoint['model']['module.0.convnext.'+key] = conv_checkpoint['model'].pop(key)

    swin_checkpoint['model'].update(conv_checkpoint['model'])

    model.load_state_dict(swin_checkpoint['model'], strict=False)
...
for epoch in range(start_epoch, epochs):
    ...
    for data, mean in tqdm(train_loader, desc='train'):
        ...
        loss = criterion(output, mean) #+ partial_rank_loss(output, mean, len(output))
        # print(loss)
    ...
    with torch.no_grad():
        ...
        for data, mean in tqdm(val_loader, desc='validation'):
            ...
            # val_loss = criterion(val_score, mean) + rank_loss(val_score, mean, len(val_score))
            val_loss = criterion(val_score, mean) #+ partial_rank_loss(val_score, mean, len(val_score))
